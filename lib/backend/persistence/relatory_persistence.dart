...
class RelatoryPersistence {
  ...
  static Future<List<FemaleBreederStatistics>> getFemaleBreedersStatistics(int pageSize, int page) async {
    ...
    for (final c in cows) {
      ...
      final futureFirstBirthDate = (database.customSelect("""
        ...
        WHERE r.cow = ? OR r.bull = ?
        ORDER BY b.date ASC
        LIMIT 1;
      """, variables: [ Variable(earring), Variable(earring) ]).getSingleOrNull());
      ...
      final futureCountFailedReproductions = database.customSelect("""
        ...
        WHERE date > (
            SELECT COALESCE((
                ...
                WHERE (r3.cow = ? OR r3.bull = ?) AND r3.diagnostic = 0
                ORDER BY r3.date DESC
                LIMIT 1
            ), 0)
        ) AND r1.cow = ?;
      """, variables: [ Variable(earring), Variable(earring), Variable(earring) ]).getSingle();
  ...
  static Future<List<FemaleBreederStatistics>> getOffspringStatistics(int earring) async {
